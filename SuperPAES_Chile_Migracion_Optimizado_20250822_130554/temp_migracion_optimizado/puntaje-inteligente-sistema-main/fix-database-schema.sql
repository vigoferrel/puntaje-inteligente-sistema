-- =====================================================
-- AGREGAR COLUMNA FALTANTE last_activity_at
-- =====================================================
...
BEGIN
    IF NOT EXISTS (
        ...
        -- Actualizar registros existentes con created_at o NOW()
        UPDATE user_node_progress 
        SET last_activity_at = COALESCE(completed_at, NOW())
...
-- =====================================================
-- FUNCIÓN production_readiness_check CORREGIDA
-- =====================================================
...
BEGIN
    ...
    -- Contar usuarios activos (usar created_at como fallback)
    SELECT COUNT(*) INTO user_count
    FROM profiles 
    WHERE COALESCE(last_active_at, created_at) > NOW() - INTERVAL '30 days';
    
    -- Actividad reciente (usar completed_at como fallback)
    SELECT COUNT(*) INTO recent_activity
    FROM user_node_progress 
    WHERE COALESCE(last_activity_at, completed_at, NOW()) > NOW() - INTERVAL '7 days';
