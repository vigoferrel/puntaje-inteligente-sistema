...
export const SkillVisualization3D: React.FC<SkillVisualization3DProps> = ({
  ...
  // SimulaciÃ³n de red 3D (en una implementaciÃ³n real usarÃ­as Three.js o similar)
  const renderSkillNetwork = () => {
    ...
    return (
      <div className="relative w-full h-96 bg-gradient-to-br from-purple-900/20 to-blue-900/20 rounded-lg overflow-hidden">
        <div className="absolute inset-0 flex items-center justify-center">
          <div className="grid grid-cols-5 gap-4 p-8">
            {nodes.map((node, index) => (
              ...
              >
                <div
                  className="w-12 h-12 rounded-full flex items-center justify-center border-2 shadow-lg transition-all dynamic-skill-node"
                  data-mastery={node.masteryLevel}
                ...
                <div className="absolute inset-0 pointer-events-none">
                  {node.prerequisites.slice(0, 2).map((prereq, preqIndex) => (
                    <div
                      key={prereq}
                      className="absolute w-px h-8 bg-cyan-400/30 dynamic-connection-line"
                      data-rotation={45 + preqIndex * 90}
                    />
                  ))}
                </div>
        ...
        {/* Efectos de partÃ­culas simulados */}
        <div className="absolute inset-0 pointer-events-none">
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <CardTitle className="text-white flex items-center gap-3">
              ...
              VisualizaciÃ³n 3D de Habilidades
            </CardTitle>
            ...
            <CardContent className="space-y-4">
              <div>
                ...
                <div className="w-full bg-gray-700 rounded-full h-2">
                  <div
                    className="h-2 rounded-full dynamic-progress-bar"
                    data-progress={selectedNode.masteryLevel}
                  />
                </div>
              </div>
                ...
                <div className="space-y-3">
                  <div className="p-3 bg-blue-600/20 rounded-lg">
                    <p className="text-white text-sm">
                      ðŸ“š Revisar conceptos fundamentales de {selectedNode.subject}
                    </p>
                  </div>
                  <div className="p-3 bg-green-600/20 rounded-lg">
                    <p className="text-white text-sm">
                      ðŸŽ¯ Practicar ejercicios de nivel {selectedNode.difficulty}
                    </p>
                  </div>
                  <div className="p-3 bg-purple-600/20 rounded-lg">
                    <p className="text-white text-sm">
                      â±ï¸ Tiempo recomendado: {selectedNode.aiRecommendations.studyTime} minutos
                    </p>
                  </div>
                </div>
              )}
