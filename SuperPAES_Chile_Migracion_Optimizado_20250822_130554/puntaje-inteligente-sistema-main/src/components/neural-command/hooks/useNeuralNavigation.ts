...
// Helper (...args: unknown[]) => unknown to validate dimension ID
...
export const useNeuralNavigation = (initialDimension: string = 'neural_command') => {
  ...
  // Activar una dimensiÃ³n especÃ­fica
  const handleDimensionActivation = useCallback((dimensionId: NeuralDimensionId) => {
    console.log(`ðŸŒŒ Activando dimensiÃ³n neural: ${dimensionId}`);
  ...
  const handleBackToCenter = useCallback(() => {
    console.log('ðŸ  Regresando al centro neural');
  ...
  // Detectar cambios de dimensiÃ³n basados en URL o props
  useEffect(() => {
