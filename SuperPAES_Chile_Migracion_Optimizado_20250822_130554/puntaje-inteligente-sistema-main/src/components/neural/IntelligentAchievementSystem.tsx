...
import { useAchievementTrigger } from './IntelligentAchievementSystem.hooks';
...
interface Achievement {
  ...
  intelligent_achievements?: Record<string, unknown>;
  unlocked_at?: string;
}
        ...
        <CardContent className="p-4">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1">
              <h3 className="font-bold text-white text-sm">Â¡Logro Desbloqueado!</h3>
...
export const IntelligentAchievementSystem: React.FC = () => {
  ...
  const [visibleNotifications, setVisibleNotifications] = useState<Achievement[]>([]);
    ...
    <div>
      <AnimatePresence>
        {visibleNotifications.map((achievement, index) => (
          <motion.div
            key={achievement.id}
            className="fixed right-4 z-50 dynamic-notification-stack"
            data-notification-bottom={`${4 + (index * 120)}px`}
          >
