...
export const NeuralAnalyticsDashboard: FC = () => {
  const {
    analytics,
    achievements,
    recommendations,
    ...
    <div className="space-y-6 p-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-300">AnÃ¡lisis avanzado de tu progreso neural</p>
        </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-black/40 border-gray-700">
          ...
          <CardContent>
            <div className="space-y-3">
              {analytics?.insights?.map((insight: unknown, index: number) => (
                <motion.div
                  key={index}
                ...
                <p className="text-gray-400 text-center py-4">
                  No hay insights disponibles. ContinÃºa usando el sistema para generar anÃ¡lisis.
                </p>
              )}
            </div>
          </CardContent>
        ...
        <Card className="bg-black/40 border-gray-700">
          ...
          <CardContent>
            <div className="space-y-3">
              {recommendations?.slice(0, 3).map((rec: unknown, index: number) => (
                <motion.div
                  key={rec.id}
                  initial={{ opacity: 0, x: 20 }}
                  animate={{ opacity: 1, x: 0 }}
                  ...
                  <div className="flex items-start justify-between">
                    <div className="flex-1">
                      <h4 className="font-semibold text-white mb-1">
                        {rec.content?.title || 'RecomendaciÃ³n'}
                      </h4>
                      <p className="text-sm text-gray-300 mb-2">
                        {rec.content?.description || 'Sin descripciÃ³n'}
                      </p>
                      {rec.content?.actions && (
