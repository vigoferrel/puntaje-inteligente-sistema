...
export class NeuralErrorBoundary extends Component<Props, State> {
  ...
  componentDidCatch(error: Error, errorInfo: ErrorInfo) {
    // Solo loguear errores crÃ­ticos para evitar spam en consola
    if (!this.props.silent && this.state.retryCount === 0) {
      console.error('ðŸ§  Neural System Error:', error.message);
            ...
            <Card className="max-w-lg mx-auto bg-black/40 backdrop-blur-xl border-red-500/30">
              <CardHeader>
                <CardTitle className="text-red-400 flex items-center gap-2">
                  ...
                  Sistema Neural RecuperÃ¡ndose
                </CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <div className="flex items-center gap-3 text-yellow-300">
                  <AlertTriangle className="w-5 h-5" />
                  <span>Detectada anomalÃ­a en el circuito neural</span>
                </div>
                
                <div className="text-white/80 text-sm">
                  El sistema estÃ¡ aplicando protocolos de auto-recuperaciÃ³n
                </div>
                ...
                <div className="text-xs text-white/50 text-center">
                  Sistema de auto-recuperaciÃ³n neural activado
                </div>
              </CardContent>
            </Card>
          </motion.div>
...
// Hook para usar error boundary programÃ¡ticamente
export const useNeuralRecovery = () => {
  ...
  const reportError = React.useCallback((error: Error) => {
    console.error('ðŸ§  Neural Recovery:', error.message);
    setError(error);
  }, []);
