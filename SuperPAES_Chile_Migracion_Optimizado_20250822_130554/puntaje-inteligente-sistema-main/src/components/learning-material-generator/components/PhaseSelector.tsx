...
export const PhaseSelector: FC<PhaseSelectorProps> = ({
  selectedPhase,
  onPhaseChange,
  currentUserPhase
}) => {
  ...
  // Validar que PHASE_CONFIG estÃ© disponible
  if (!PHASE_CONFIG || typeof PHASE_CONFIG !== 'object') {
    return (
      <Alert variant="destructive">
        <AlertCircle className="h-4 w-4" />
        <AlertDescription>
          Error al cargar las fases del ciclo de aprendizaje. Por favor, recarga la pÃ¡gina.
        </AlertDescription>
      </Alert>
    );
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 gap-2">
        {mainPhases.map((phase) => {
          ...
          // ValidaciÃ³n adicional por si alguna fase no tiene configuraciÃ³n
          if (!config) {
            console.warn('ConfiguraciÃ³n no encontrada para fase:', phase);
            return null;
          }
                  ...
                  {config.recommendedMaterials.slice(0, 2).map((material) => (
                    <Badge key={material} variant="secondary" className="text-xs px-1">
                      {material === 'exercises' ? 'Ejercicios' :
                       ...
                       material === 'diagnostic_tests' ? 'EvaluaciÃ³n' :
                       material === 'practice_guides' ? 'GuÃ­as' : 'Simulacros'}
                    </Badge>
                  ))}
                </div>
              </CardContent>
