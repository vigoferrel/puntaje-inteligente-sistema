...
export const CinematicTransition: FC<CinematicTransitionProps> = ({
  isActive,
  currentFlow,
  targetFlow,
  progress = 0,
    ...
    <AnimatePresence>
      ...
      >
        <div className="text-center space-y-8">
          ...
          {/* Indicador de progreso cinematogrÃ¡fico */}
...
(...args: unknown[]) => unknown getFlowDisplayName(flow: string): string {
  const names = {
...
(...args: unknown[]) => unknown getContextDisplayName(context: string): string {
  if (context.includes(':')) {
    const [type, detail] = context.split(':');
    const contextNames = {
      evaluation: `Preparando evaluaciÃ³n de ${detail}`,
      ...
      history: `Cargando anÃ¡lisis de ${detail}`,
      dashboard: `Actualizando mÃ©tricas de ${detail}`,
      diagnostic: `Configurando diagnÃ³stico ${detail}`
    };
