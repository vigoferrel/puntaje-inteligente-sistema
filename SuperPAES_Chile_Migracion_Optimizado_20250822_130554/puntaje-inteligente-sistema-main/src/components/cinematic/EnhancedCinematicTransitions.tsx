...
export const EnhancedCinematicTransitions: FC<EnhancedCinematicTransitionsProps> = ({
  children,
  variant = 'neural',
  duration = 1,
  className = ''
  ...
  return (
    <motion.div
      ...
      animate="animate"
      exit="exit"
      className={`${className} transform-gpu will-change-transform dynamic-style`}
      data-style="{perspective: '1000px', transformStyle: 'preserve-3d'}"
    >
      {children}
      
      {/* Efectos de partÃ­culas durante la transiciÃ³n */}
      <motion.div
      ...
      >
        {Array.from({ length: 12 }).map((_, i) => (
          <motion.div
            key={i}
            className={`absolute w-1 h-1 rounded-full dynamic-particle ${
