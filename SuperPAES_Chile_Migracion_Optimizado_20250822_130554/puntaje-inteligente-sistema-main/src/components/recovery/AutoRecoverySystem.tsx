...
export const AutoRecoverySystem: React.FC<AutoRecoverySystemProps> = ({ children }) => {
  ...
  useEffect(() => {
    const handleError = (error: ErrorEvent) => {
      console.error('ðŸš¨ Error detectado por AutoRecoverySystem:', error);
    ...
    const handleUnhandledRejection = (event: PromiseRejectionEvent) => {
      console.error('ðŸš¨ Promise rejection detectada:', event.reason);
      setErrorCount(prev => prev + 1);
    };
      ...
      <div className="min-h-screen bg-gradient-to-br from-red-900 via-purple-900 to-indigo-900 flex items-center justify-center p-4">
        <Card className="max-w-md w-full bg-red-950/50 border-red-500/30">
          <CardContent className="p-6 text-center space-y-4">
            ...
            <h2 className="text-xl font-bold text-white">Sistema en RecuperaciÃ³n</h2>
            <p className="text-red-300">
              Se detectaron mÃºltiples errores. Iniciando proceso de recuperaciÃ³n automÃ¡tica.
            </p>
            <Button 
