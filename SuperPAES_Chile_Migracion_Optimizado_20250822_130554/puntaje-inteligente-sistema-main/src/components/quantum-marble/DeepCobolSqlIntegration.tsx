...
export const DeepCobolSqlIntegration: React.FC = () => {
  ...
  const achieveQuantumTranscendence = useCallback(async () => {
    ...
    const avgMarbleReflection = Object.values(marbleReflection).flatMap(aspect => Object.values(aspect)).reduce((a, b) => a + b, 0) / 12;
                  ...
                  {Object.entries(aspectValue).map(([subKey, subValue]) => (
                    <div key={subKey} className="flex flex-col gap-1">
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-purple-400 text-xs w-8">{subValue.toFixed(0)}%</span>
                      </div>
                    </div>
                  ))}
                </div>
...
export default DeepCobolSqlIntegration;