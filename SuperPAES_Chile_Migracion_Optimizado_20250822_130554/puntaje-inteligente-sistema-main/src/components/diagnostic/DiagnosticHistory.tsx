...
export const DiagnosticHistory = ({ onSelectResult, className = "" }: DiagnosticHistoryProps) => {
  ...
  // Ordenar resultados por fecha (mÃ¡s reciente primero)
  const sortedResults = [...results].sort(
    (a, b) => new Date(b.completedAt).getTime() - new Date(a.completedAt).getTime()
  );
  
  // Obtener el resultado mÃ¡s reciente
  const latestResult = sortedResults.length > 0 ? sortedResults[0] : null;
  
  // Preparar datos para el grÃ¡fico de progreso
  const prepareChartData = () => {
    if (results.length === 0) return [];
    
    // Obtener todas las habilidades Ãºnicas de todos los resultados
    ...
    // Crear datos para el grÃ¡fico ordenados por fecha (mÃ¡s antiguo primero)
    return [...results]
      .sort((a, b) => new Date(a.completedAt).getTime() - new Date(b.completedAt).getTime())
      .map(result => {
        const chartPoint: unknown = {
          date: format(new Date(result.completedAt), 'dd/MM/yy'),
          timestamp: new Date(result.completedAt).getTime()
        };
        
        // AÃ±adir cada habilidad como una propiedad
  ...
  const getSkillColor = (skill: TPAESHabilidad, index: number): string => {
    ...
    // Asignar color segÃºn Ã­ndice o usar un color predeterminado si no hay suficientes
    return colors[index % colors.length];
  };
  
  // Renderizar lÃ­neas para cada habilidad
  const renderLines = () => {
    ...
    return [
      ...
      <Line
        ...
        name="Promedio"
      />,
      // Luego todas las demÃ¡s habilidades
  ...
  if (error) {
    return (
      <Card className={className}>
        <CardHeader>
          <CardTitle>Historial de diagnÃ³sticos</CardTitle>
    ...
    return (
      <Card className={className}>
        <CardHeader>
          ...
          <CardDescription>
            No hay diagnÃ³sticos completados
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col items-center justify-center text-center py-8">
          <ClipboardList className="h-16 w-16 text-muted-foreground mb-4 opacity-30" />
          <p className="text-muted-foreground mb-4">
            No has completado ningÃºn diagnÃ³stico todavÃ­a. 
            Completa un diagnÃ³stico para ver tu progreso aquÃ­.
          </p>
        </CardContent>
      </Card>
    );
              ...
              {chartData.length > 1 ? (
                <div className="space-y-4">
                  ...
                  <div className="flex justify-between items-center text-xs text-muted-foreground border-t pt-2">
                    ...
                    <div>
                      Total: {results.length} diagnÃ³stico{results.length !== 1 ? 's' : ''}
                    </div>
                  </div>
                </div>
              ) : (
                <div className="text-center py-4">
                  <p className="text-muted-foreground mb-2">
                    Necesitas al menos 2 diagnÃ³sticos para ver tu progreso.
                  </p>
                  {latestResult && (
                    <div className="mt-4">
                      <h3 className="font-medium mb-2">Tu diagnÃ³stico mÃ¡s reciente:</h3>
                          ...
                          <div className="space-y-2">
                            ...
                            {Object.keys(result.results).length > 3 && (
                              <p className="text-xs text-muted-foreground text-right">
                                +{Object.keys(result.results).length - 3} mÃ¡s
                              </p>
                            )}
                          </div>
                        </div>
              ...
              <TabsContent value="latest">
                <div className="space-y-4">
                  <div className="flex justify-between items-center">
                    <div>
                      <h3 className="font-medium">Ãšltimo diagnÃ³stico</h3>
