...
export const InteractiveNodeGroup: React.FC<InteractiveNodeGroupProps> = ({
  ...
  const getNodeColor = (node: unknown) => {
    ...
    if (hovered === node.id) return '#8B5CF6'; // PÃºrpura para hover
  ...
  // PartÃ­culas alrededor de nodos completados
  ...
  return (
    <group ref={groupRef}>
      ...
      {/* Indicador de Progreso de la ConstelaciÃ³n */}
      ...
      {constellation.nodes.map((node, index) => (
        <group key={node.id}>
          ...
          {/* PartÃ­culas para nodos completados */}
