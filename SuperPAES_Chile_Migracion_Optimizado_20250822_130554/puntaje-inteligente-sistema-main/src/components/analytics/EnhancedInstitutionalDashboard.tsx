    ...
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <div>
          ...
          <p className="text-slate-600">Sistema unificado con mÃ©tricas predictivas</p>
        </div>
      ...
      >
        <Card>
          <CardHeader>
            <CardTitle>DistribuciÃ³n de Riesgo AcadÃ©mico</CardTitle>
          </CardHeader>
          <CardContent>
      ...
      {/* MÃ©tricas predictivas (si estÃ¡n disponibles) */}
      {(metrics as unknown).predictive && (
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          <Card className="border-2 border-blue-200 bg-gradient-to-r from-blue-50 to-indigo-50">
            <CardHeader>
              <CardTitle className="text-blue-800">MÃ©tricas Predictivas</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div className="text-center">
                  ...
                  <Badge className={`
                    ${(metrics as unknown).predictive.performanceTrend === 'increasing' ? 'bg-green-500' : 
                      (metrics as unknown).predictive.performanceTrend === 'stable' ? 'bg-yellow-500' : 'bg-red-500'}
                  `}>
                    {(metrics as unknown).predictive.performanceTrend === 'increasing' ? 'â†— Mejorando' :
                     (metrics as unknown).predictive.performanceTrend === 'stable' ? 'â†’ Estable' : 'â†˜ Disminuyendo'}
                  </Badge>
                </div>
                <div className="text-center">
                  <p className="text-sm text-blue-600">Estudiantes en Riesgo de DeserciÃ³n</p>
                  <p className="text-2xl font-bold text-blue-800">
                    {(metrics as unknown).predictive.riskPrediction.predictedDropouts}
                  </p>
                </div>
                <div className="text-center">
                  <p className="text-sm text-blue-600">Probabilidad de Ã‰xito</p>
                  <p className="text-2xl font-bold text-blue-800">
                    {Math.round((metrics as unknown).predictive.goalProjection.successProbability)}%
                  </p>
                </div>
              </div>
            </CardContent>
