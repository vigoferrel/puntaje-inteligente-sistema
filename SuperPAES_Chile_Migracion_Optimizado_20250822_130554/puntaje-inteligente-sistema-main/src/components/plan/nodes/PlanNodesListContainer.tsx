  ...
  if (!nodes || nodes.length === 0) {
    return (
      <div className="text-center py-8 text-muted-foreground">
        <p>No hay mÃ³dulos disponibles en este plan.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-3">
        <h3 className="font-medium text-lg">MÃ³dulos de estudio</h3>
        
        <div className="flex flex-wrap gap-2 items-center">
          {/* Study statistics badges */}
          <Badge variant="outline" className="flex items-center gap-1">
            <BookOpen className="h-3 w-3" />
            <span>{nodes.length} mÃ³dulos</span>
          </Badge>
      ...
      {filteredNodes.length === 0 ? (
        <div className="text-center py-8 text-muted-foreground border rounded-md p-4">
          <p>No hay mÃ³dulos que coincidan con el filtro seleccionado.</p>
          <Button 
            variant="link" 
