...
export const StudyStreakCard = ({ streakData }: StudyStreakCardProps) => {
  ...
  const getMotivationalMessage = (streak: number) => {
    if (streak >= 10) return "Â¡IncreÃ­ble disciplina! Sigue asÃ­.";
    if (streak >= 7) return "Â¡Tu constancia estÃ¡ dando frutos!";
    if (streak >= 3) return "Â¡Buen ritmo! MantÃ©n la constancia.";
    if (streak >= 1) return "Â¡Buen comienzo! Un dÃ­a a la vez.";
    return "Â¡Comienza hoy tu racha de estudio!";
  };
                ...
                <div className="flex-1">
                  ...
                  <div className="flex items-center">
                    <p className="text-sm text-muted-foreground">
                      {getCurrentStreak() > 0 ? 
                        `${getCurrentStreak()} dÃ­a${getCurrentStreak() !== 1 ? 's' : ''} consecutivo${getCurrentStreak() !== 1 ? 's' : ''}` : 
                        "Comienza hoy"}
                    </p>
                  </div>
                </div>
                ...
                <div className="mt-3">
                  <div className="flex justify-between items-center text-xs mb-1">
                    <span className="font-medium text-primary/80">
                      {daysUntilNextBadge} dÃ­a{daysUntilNextBadge !== 1 ? 's' : ''} para el siguiente nivel
                    </span>
              ...
              {daysSinceLastStudy !== null && daysSinceLastStudy > 0 && (
                <p className="text-xs text-muted-foreground mt-2">
                  {daysSinceLastStudy === 1 ? 
                    "Â¡No pierdas tu racha! Estudia hoy." : 
                    `Han pasado ${daysSinceLastStudy} dÃ­as desde tu Ãºltima sesiÃ³n.`}
                </p>
              )}
            </CardContent>
          </Card>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-2 gap-4 text-sm">
              <div className="bg-primary/5 p-2 rounded">
                ...
                <p className="text-lg font-semibold">{streakData?.currentStreak || 0} dÃ­as</p>
              </div>
              <div className="bg-primary/5 p-2 rounded">
                <div className="flex items-center text-primary mb-1">
                ...
                <p className="text-lg font-semibold">{streakData?.longestStreak || 0} dÃ­as</p>
              </div>
              <div className="bg-primary/5 p-2 rounded">
                <div className="flex items-center text-primary mb-1">
                  <CalendarDays className="h-4 w-4 mr-1" />
                  <p className="font-medium">Ãšltimo estudio</p>
                </div>
              ...
              <div className="bg-primary/5 p-2 rounded">
                <div className="flex items-center text-primary mb-1">
                  ...
                  <p className="font-medium">Total dÃ­as</p>
                </div>
                <p className="font-medium">{streakData?.totalStudyDays || 0} dÃ­as</p>
              </div>
            </div>
            <p className="text-xs text-muted-foreground">
              Estudiar todos los dÃ­as mejora tus resultados hasta un 80%. Â¡MantÃ©n tu racha!
            </p>
          </div>
        </TooltipContent>
      </Tooltip>
