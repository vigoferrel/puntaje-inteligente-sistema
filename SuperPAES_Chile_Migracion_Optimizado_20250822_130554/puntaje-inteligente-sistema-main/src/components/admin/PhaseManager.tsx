...
export const PhaseManager = () => {
  ...
  const handleSetPhase = async () => {
    if (!profile?.id) {
      toast({
        title: "Error",
        description: "Debes iniciar sesiÃ³n para cambiar la fase",
        variant: "destructive"
      });
      return;
    }
  ...
  const handleAdvancePhase = async () => {
    if (!profile?.id) {
      toast({
        title: "Error",
        description: "Debes iniciar sesiÃ³n para avanzar de fase",
        variant: "destructive"
      });
      return;
    }
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>GestiÃ³n de Fase de Aprendizaje</CardTitle>
        <CardDescription>Cambia o avanza la fase actual de aprendizaje</CardDescription>
      </CardHeader>
      <CardContent className="space-y-4">
        <div className="space-y-2">
          <label className="text-sm font-medium">Seleccionar fase especÃ­fica:</label>
