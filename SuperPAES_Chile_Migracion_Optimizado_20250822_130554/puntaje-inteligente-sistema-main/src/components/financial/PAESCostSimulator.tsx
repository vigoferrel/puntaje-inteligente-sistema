...
export const PAESCostSimulator: React.FC<PAESCostSimulatorProps> = ({ userProfile }) => {
  ...
  const universities = [
    ...
    { id: 'puc', name: 'Pontificia Universidad CatÃ³lica', type: 'private', prestige: 'high' },
  ...
  const careers = [
    ...
    { id: 'ingenieria', name: 'IngenierÃ­a Civil', duration: 6, avgCost: 4500000, cutoffScore: 650 },
    { id: 'derecho', name: 'Derecho', duration: 5, avgCost: 3500000, cutoffScore: 600 },
    { id: 'psicologia', name: 'PsicologÃ­a', duration: 5, avgCost: 3000000, cutoffScore: 580 }
  ];
                ...
                <SelectContent>
                  {universities.map(uni => (
                    <SelectItem key={uni.id} value={uni.id}>
                      {uni.name} ({uni.type === 'public' ? 'PÃºblica' : 'Privada'})
                    </SelectItem>
                  ))}
                </SelectContent>
              </Select>
          ...
          <Card className="bg-black/40 backdrop-blur-xl border-white/20">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <h4 className="text-white font-medium mb-3">Costos AcadÃ©micos</h4>
                  <div className="space-y-2">
                    <div className="flex justify-between">
                      <span className="text-white/70">MatrÃ­cula anual:</span>
                      <span className="text-white">${simulation.annualCost.toLocaleString()}</span>
                    </div>
                    <div className="flex justify-between">
                      <span className="text-white/70">DuraciÃ³n carrera:</span>
                      <span className="text-white">{simulation.career.duration} aÃ±os</span>
                    </div>
                    <div className="flex justify-between font-bold">
                      <span className="text-white">Total acadÃ©mico:</span>
              ...
              <div className="border-t border-white/20 pt-4">
                <div className="flex justify-between items-center text-xl font-bold">
                  <span className="text-white">InversiÃ³n Total Proyectada:</span>
