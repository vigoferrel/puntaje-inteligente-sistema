...
class OptimizedMemoryManager {
  ...
  markModuleLoaded(moduleName: string) {
    ...
    if (process.env.NODE_ENV === 'development') {
      console.log(`ðŸ“¦ MÃ³dulo cargado: ${moduleName}`);
    }
  }
...
export const useOptimizedPreloading = (currentRoute: string) => {
  ...
  useEffect(() => {
    // Solo preload en rutas especÃ­ficas y con delay mÃ¡s alto
    ...
    if (modulesToPreload.length > 0 && 'requestIdleCallback' in window) {
      requestIdleCallback(() => {
        ...
        // Solo preload si no hay muchos mÃ³dulos cargados
        if (loadedCount < 3) {
          if (process.env.NODE_ENV === 'development') {
            console.log(`ðŸš€ Preloading para ${currentRoute}:`, modulesToPreload);
          }
        }
      }, { timeout: 5000 });
    }
