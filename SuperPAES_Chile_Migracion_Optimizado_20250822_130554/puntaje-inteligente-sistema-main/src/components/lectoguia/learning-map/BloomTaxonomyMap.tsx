...
const BLOOM_LEVELS = [
  { name: 'Recordar', description: 'Recuperar informaciÃ³n relevante', color: 'bg-red-100 text-red-800' },
  { name: 'Comprender', description: 'Construir significado a partir de la informaciÃ³n', color: 'bg-orange-100 text-orange-800' },
...
export const BloomTaxonomyMap: FC<BloomTaxonomyMapProps> = ({
  nodes,
  nodeProgress,
  skillLevels,
  selectedPrueba,
  ...
  // Simular distribuciÃ³n de nodos por nivel de Bloom
  const getBloomDistribution = () => {
    const distribution = BLOOM_LEVELS.map(level => ({
      ...level,
      nodes: nodes.filter(() => Math.random() > 0.7), // SimulaciÃ³n temporal
  ...
  return (
    <div className="space-y-6">
      <div className="text-center space-y-2">
        <h3 className="text-2xl font-bold">TaxonomÃ­a de Bloom</h3>
        <p className="text-muted-foreground">
          Progresa a travÃ©s de los niveles cognitivos de aprendizaje
        </p>
      </div>
