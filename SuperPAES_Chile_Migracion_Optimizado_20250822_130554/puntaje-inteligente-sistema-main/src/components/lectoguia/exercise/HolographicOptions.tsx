...
export const HolographicOptions: FC<HolographicOptionsProps> = ({
  options,
  selectedOption,
  correctAnswerIndex,
  showFeedback,
      ...
      {options.map((option, index) => {
        ...
        return (
          ...
          >
            ...
            {/* BotÃ³n principal con fondo mÃ¡s opaco */}
            <motion.button
            ...
            >
              {/* PartÃ­culas flotantes para opciones especiales */}
              {(shouldHighlightCorrect || isSelected) && (
                <div className="absolute inset-0 pointer-events-none">
                  {[...Array(5)].map((_, i) => (
                    <motion.div
                      key={i}
                      className={`absolute w-1 h-1 rounded-full dynamic-floating-particle ${
                        shouldHighlightCorrect ? 'bg-green-400' : 'bg-primary'
                      }`}
                      data-particle-left={`${20 + i * 15}%`}
                      data-particle-top={`${30 + Math.sin(i) * 20}%`}
                      animate={{
                        y: [0, -10, 0],
                        opacity: [0.7, 1, 0.7]
                      }}
              ...
              <div className="flex items-start gap-4">
                {/* Indicador de letra con fondo sÃ³lido */}
                ...
                {/* Texto de la opciÃ³n con mejor contraste */}
                ...
                {/* Indicador de selecciÃ³n activa */}
