...
export const ContextualBackground: FC<ContextualBackgroundProps> = ({ prueba }) => {
    ...
    <div className="absolute inset-0 overflow-hidden">
      ...
      <div className="absolute inset-0">
        {[...Array(3)].map((_, i) => (
          <motion.div
            key={i}
            className={`absolute rounded-full ${getPatternColor().replace('/10', '/5')} blur-3xl dynamic-floating-wave`}
            data-wave-width={`${300 + i * 100}px`}
            data-wave-height={`${300 + i * 100}px`}
            data-wave-left={`${20 + i * 30}%`}
            data-wave-top={`${10 + i * 20}%`}
            animate={{
              x: [0, 50, 0],
              y: [0, -30, 0],
              scale: [1, 1.1, 1]
