...
import { ExtendedNeuralNode, EducationalGalaxy, GalaxyComponentProps } from '../../types/neural-extended';
...
const RealGalaxy: React.FC<RealGalaxyProps> = ({ galaxy, isSelected, onClick }) => {
  ...
  return (
    <group position={galaxy.position}>
      ...
      >
        <motion.div 
          className="bg-black/90 backdrop-blur-xl rounded-lg p-3 text-white text-center border min-w-40 dynamic-border"
          initial={{ opacity: 0, scale: 0.8 }}
          animate={{ opacity: 1, scale: 1 }}
          whileHover={{ scale: 1.1 }}
          data-border={isSelected ? '#00FFFF' : galaxy.color}
        >
          <div className="text-sm font-bold dynamic-color" data-color={galaxy.color}>
            {galaxy.name}
          </div>
          ...
          <div className="text-xs mt-1">
            <span className="dynamic-color" data-color={galaxy.color}>
...
const InterGalacticConnections: React.FC<{ galaxies: EducationalGalaxy[] }> = ({ galaxies }) => {
...
const Educational2DFallback = ({ galaxies, onGalaxyClick }: GalaxyComponentProps) => (
  <div className="w-full h-[600px] bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 rounded-lg p-6 flex flex-wrap gap-4 items-center justify-center overflow-auto">
    {galaxies.map((galaxy, index) => (
      <motion.div
        ...
        className="bg-black/40 backdrop-blur-sm rounded-lg p-4 border border-white/20 cursor-pointer hover:bg-black/60 transition-all dynamic-border"
        onClick={() => onGalaxyClick?.(galaxy.testCode)}
        data-border={galaxy.color}
      >
        <div className="text-center">
          <div className="text-lg font-bold mb-2 dynamic-color" data-color={galaxy.color}>
            {galaxy.name}
          </div>
          ...
          <div className="w-full bg-gray-700 rounded-full h-2">
            <div 
              className="h-2 rounded-full transition-all dynamic-progress-bar"
              data-bg-color={galaxy.color}
              data-width={`${(galaxy.completed / galaxy.nodes) * 100}%`}
            />
          </div>
        </div>
      </motion.div>
...
export const RealEducationalUniverse: React.FC<RealEducationalUniverseProps> = ({
  ...
  const { fallbackMode } = useNeuralCinematicLoading('RealEducationalUniverse');
  ...
  const galaxies = useMemo(() => {
    ...
    realNodes.forEach(node => {
      const nodeWithTest = node as any;
      const testKey = `test_${nodeWithTest.testId || node.id}`;
      if (!testMap.has(testKey)) {
        testMap.set(testKey, {
          id: testKey,
          testId: nodeWithTest.testId || node.id,
          nodes: [],
          completed: 0
        });
      }
    ...
    const testNames = {
      ...
      'test_2': 'MatemÃ¡tica M1', 
      'test_3': 'MatemÃ¡tica M2',
      'test_4': 'Historia',
      'test_5': 'Ciencias'
    };
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Panel de mÃ©tricas universales */}
