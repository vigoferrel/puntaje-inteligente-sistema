...
export class SmartExerciseBankPAES {
  ...
  private static cache = new Map<string, { data: BankStats; timestamp: number }>();
  ...
  private static async getFromBank(
    ...
    if (exercises && exercises.length > 0) {
      ...
      return exercises[randomIndex] as DatabaseExercise;
    }
  ...
  private static async getAdaptiveExercise(
    ...
    return exercises[0] as DatabaseExercise;
  }
   ...
   */
  private static convertToExerciseFormat(dbExercise: DatabaseExercise): Exercise {
    const metadata = dbExercise.metadata || {};
   ...
   */
  private static async getUserMetrics(userId: string): Promise<UserMetrics> {
  ...
  static async getBankStats(): Promise<BankStats> {
    ...
    try {
      ...
      for (const exercise of exerciseList) {
        // Contar por materia
        if (exercise.prueba_paes && Object.prototype.hasOwnProperty.call(bySubject, exercise.prueba_paes)) {
          bySubject[exercise.prueba_paes as TPAESPrueba]++;
        }

        // Contar por dificultad
        if (exercise.difficulty_level && Object.prototype.hasOwnProperty.call(byDifficulty, exercise.difficulty_level)) {
    ...
    try {
      const { error } = await supabase
        ...
        .insert({
          ...
          options: exercise.options as unknown,
  ...
  // Métodos de cache
  private static getCachedData(key: string): BankStats | null {
  ...
  private static setCachedData(key: string, data: BankStats): void {
    this.cache.set(key, {
      data,
      timestamp: Date.now()
    });
  ...
  private static getMetricValue(metrics: MetricRecord[], metricType: string): number | null {
    const metric = metrics?.find((m: MetricRecord) => m.metric_type === metricType);
    return metric ? metric.current_value : null;
  }

  // ?? SIMBIOSIS: Sistema anti-repetición
  private static userHistory = new Map<string, Set<string>>();

  static isUnique(exercise: Exercise, userId: string): boolean {
    const hash = btoa(`${exercise.question}${exercise.skill}`).substring(0, 16);
    const history = this.userHistory.get(userId) || new Set();
    return !history.has(hash);
  }

  static markUsed(exercise: Exercise, userId: string): void {
    const hash = btoa(`${exercise.question}${exercise.skill}`).substring(0, 16);
    const history = this.userHistory.get(userId) || new Set();
    history.add(hash);
    if (history.size > 50) {
      const firstValue = history.values().next().value;
      if (firstValue) history.delete(firstValue);
    }
    this.userHistory.set(userId, history);
  }

  // ?? FUNCIONALIDAD ADICIONAL: Predicción inteligente
  static predictNextExercise(userId: string, currentSkill: string): string[] {
    const history = this.userHistory.get(userId) || new Set();
    const skillProgression = ['BASICO', 'INTERMEDIO', 'AVANZADO'];
    return skillProgression.filter(level => !Array.from(history).some(h => h.includes(level)));
  }

  // ?? FUNCIONALIDAD ADICIONAL: Auto-limpieza inteligente
  static autoCleanup(): void {
    if (this.userHistory.size > 1000) this.userHistory.clear();
  }
}
