...
export class ExerciseMassGenerationService {
  ...
  /**
   * Limpieza quirÃºrgica: elimina ejercicios generados, preserva oficiales
   */
  private static async performSurgicalCleanup(): Promise<void> {
    logger.info('ExerciseMassGenerationService', 'Ejecutando limpieza quirÃºrgica');
    ...
    try {
      ...
      // Limpiar cachÃ© del banco inteligente
      ...
      if (cacheError) {
        logger.warn('ExerciseMassGenerationService', 'Error limpiando cachÃ©', cacheError);
      }
      
      logger.info('ExerciseMassGenerationService', 'Limpieza quirÃºrgica completada');
    } catch (error) {
      logger.error('ExerciseMassGenerationService', 'Error en limpieza quirÃºrgica', error);
      throw error;
    }
  }
   ...
   */
  private static async fetchAllLearningNodes(): Promise<unknown[]> {
   ...
   */
  private static async generateStratifiedExercises(nodes: unknown[]): Promise<GenerationStats> {
  ...
  /**
   * Determina cantidad de ejercicios segÃºn tier del nodo
   */
  ...
  /**
   * Genera ejercicios para un nodo especÃ­fico
   */
  private static async generateExercisesForNode(
    ...
    // Distribuir ejercicios por dificultad: 40% bÃ¡sico, 45% intermedio, 15% avanzado
    const difficulties = [
  ...
  private static async saveExerciseToDatabase(
    ...
    const { error } = await supabase
      ...
      .insert({
        ...
        explanation: exercise.explanation || 'ExplicaciÃ³n generada automÃ¡ticamente',
  ...
  private static async updateSystemMetrics(stats: GenerationStats): Promise<void> {
    ...
    if (error) {
      logger.warn('ExerciseMassGenerationService', 'Error actualizando mÃ©tricas', error);
    }
  }
}
