  ...
  static async generateNodeRecommendations(userId: string) {
    try {
      ...
      return nodes.map(node => ({
        ...
        recommendationReason: 'Ãrea de mejora identificada'
      }));
    } catch (error) {
...
export async (...args: unknown[]) => unknown getUserPAESAnalytics(userId: string, prueba?: TPAESPrueba): Promise<PAESAnalytics[]> {
  try {
...
export async (...args: unknown[]) => unknown getSkillPerformanceAnalytics(userId: string, skill: TPAESHabilidad): Promise<{
  totalAttempts: number;
  correctAttempts: number;
  accuracy: number;
