...
/**
 * Codigo unico del examen PAES Historia consolidado
 */
const PAES_HISTORIA_EXAM_CODE = 'PAES_HISTORIA_2024_FORMA_123';

/**
 * Obtiene el examen completo de PAES Historia 2024 usando la funcion especifica de historia
 */
export const fetchPAESHistoriaExam = async (): Promise<PAESHistoriaExamComplete | null> => {
  try {
    console.log('ðŸ” Fetching complete PAES Historia exam (65 questions)');
    
    // Usar la funcion especifica para historia con el codigo correcto
    ...
    if (!data) {
      console.warn('No data returned from Historia RPC (...args: unknown[]) => unknown');
      return await fetchHistoriaExamDirect();
    }
...
const fetchHistoriaExamDirect = async (): Promise<PAESHistoriaExamComplete | null> => {
  try {
    console.log('ðŸ”„ Using direct query fallback for Historia exam');
...
/**
 * Obtiene preguntas por secciÃ³n especifica
 */
...
/**
 * Obtiene estadisticas del examen de Historia
 */
...
export const validateHistoriaExamIntegrity = async (): Promise<{
  ...
  try {
    ...
    examData.preguntas.forEach(pregunta => {
      ...
      // La pregunta 56 debe tener 5 opciones, las demÃ¡s 4
...
/**
 * Obtiene el codigo del examen PAES Historia actual
 */
