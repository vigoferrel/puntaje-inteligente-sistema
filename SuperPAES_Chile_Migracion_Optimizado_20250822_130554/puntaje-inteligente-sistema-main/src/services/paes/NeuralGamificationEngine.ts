...
interface Achievement {
  ...
  points: number;
  condition: (userStats: unknown) => boolean;
}
...
export class NeuralGamificationEngine {
  ...
  /**
   * Procesa logros y puntos despuÃ©s de completar un ejercicio
   */
  static async processExerciseCompletion(
    userId: string, 
    exerciseResult: unknown
  ): Promise<GamificationMetrics> {
    logger.info('NeuralGamificationEngine', 'Procesando finalizaciÃ³n de ejercicio', {
      userId, 
      isCorrect: exerciseResult.isCorrect
    });
    
    try {
      // Obtener estadÃ­sticas actuales del usuario
      ...
      logger.error('NeuralGamificationEngine', 'Error procesando gamificaciÃ³n', error);
      throw error;
    }
  }
   ...
   */
  private static calculateExercisePoints(result: unknown): number {
    let basePoints = 10;
    ...
    // Bonus por tiempo (si respondiÃ³ rÃ¡pido)
  ...
  /**
   * Obtiene estadÃ­sticas actuales del usuario
   */
  private static async getUserStats(userId: string): Promise<unknown> {
   ...
   */
  private static async checkAndUnlockAchievements(userId: string, userStats: unknown): Promise<Achievement[]> {
    const allAchievements = this.getAllAchievements();
    const userAchievementIds = new Set(userStats.achievements.map((a: unknown) => a.achievement_id));
    ...
    return [
      {
        ...
        rarity: 'common',
        points: 50,
        condition: (stats) => stats.metrics.some((m: unknown) => 
      ...
      },
      {
        id: 'streak_5',
        title: 'En Racha',
        description: 'MantÃ©n una racha de 5 ejercicios correctos',
        category: 'racha',
        rarity: 'rare',
        points: 100,
      ...
      },
      {
        id: 'streak_10',
        title: 'Imparable',
        description: 'MantÃ©n una racha de 10 ejercicios correctos',
        category: 'racha',
        rarity: 'epic',
        points: 250,
      ...
      {
        id: 'math_master',
        title: 'Maestro MatemÃ¡tico',
        description: 'Completa 50 ejercicios de matemÃ¡ticas',
        category: 'materia',
        rarity: 'epic',
        points: 300,
        condition: (stats) => stats.metrics.some((m: unknown) => 
      ...
      },
      {
        ...
        description: 'Completa 50 ejercicios de comprensiÃ³n lectora',
        category: 'materia',
        rarity: 'epic',
        points: 300,
        condition: (stats) => stats.metrics.some((m: unknown) => 
      ...
      },
      {
        ...
        description: 'ObtÃ©n 100% de aciertos en 20 ejercicios consecutivos',
        category: 'precision',
        rarity: 'legendary',
        points: 500,
        condition: (stats) => stats.metrics.some((m: unknown) => 
   ...
   */
  private static async updateStreak(userId: string, userStats: unknown): Promise<void> {
  ...
  /**
   * Calcula mÃ©tricas finales de gamificaciÃ³n
   */
