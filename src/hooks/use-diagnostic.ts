...
import { mapSkillIdToEnum, mapEnumToSkillId, mapTestIdToEnum } from "@/utils/supabase-mappers";
...
export const useDiagnostic = () => {
  ...
  const fetchDiagnosticTests = async (userId: string) => {
    try {
      ...
      // Fetch real questions from the database instead of using mock data
      const testsWithQuestions: DiagnosticTest[] = await Promise.all(
        testData.map(async (test) => {
          ...
          // For now, use our improved mock questions generator that uses proper mapping
  ...
  const fetchDiagnosticQuestions = async (diagnosticId: string, testId: number): Promise<DiagnosticQuestion[]> => {
    try {
      // Try to fetch real questions from the database
