    ...
    try {
      ...
      if (securityData.overall_status === 'warning') {
        newAlerts.push({
          id: 'system-warning',
          type: 'warning', 
          title: 'Sistema Requiere Atención',
          message: `Performance del sistema en ${securityData.performance_score}%`,
          severity: 'medium',
          timestamp: Date.now(),
          actions: ['Optimizar consultas', 'Revisar índices']
        });
      } else if (securityData.overall_status === 'critical') {
        newAlerts.push({
          id: 'system-critical',
          type: 'error',
          title: 'Estado Crítico del Sistema', 
          message: 'Se requiere intervención inmediata',
          severity: 'high',
          timestamp: Date.now(),
          actions: ['Contactar soporte', 'Ejecutar diagnóstico completo']
        });
      }
