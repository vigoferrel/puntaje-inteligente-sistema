...
export const NeuralAssistant: React.FC<NeuralAssistantProps> = ({
  ...
  const [messages, setMessages] = useState<Array<{id: string, text: string, type: 'ai' | 'user'}>>([
    {
      id: '1',
      text: `¡Bienvenido al Universo Neural PAES! Tu progreso actual es del ${userMetrics.overallProgress}%. ¿En qué galaxia te gustaría enfocar tu aprendizaje?`,
      type: 'ai'
    }
  ]);
  ...
  const getContextualMessage = () => {
    switch (mode) {
      case 'neural':
        return "El Centro Neural SuperPAES está analizando tu perfil vocacional. Basándome en tu progreso, puedo recomendarte carreras alineadas con tus fortalezas.";
      case 'subject':
        return selectedGalaxy 
          ? `Explorando la ${selectedGalaxy}. Veo que has completado ${userMetrics.totalCompleted} nodos. ¿Te gustaría que genere ejercicios específicos?`
          : "Selecciona una galaxia para comenzar tu viaje de aprendizaje personalizado.";
      case 'prediction':
        return `Con tu proyección actual de ${userMetrics.projectedScore} puntos PAES, tienes acceso a múltiples carreras. ¿Quieres explorar opciones específicas?`;
      case 'progress':
        return "Tu evolución temporal muestra un crecimiento consistente. Las áreas de mayor potencial están iluminándose en la nebulosa.";
      default:
                  ...
                  <div className="flex items-center justify-between text-white">
                    <div className="flex items-center space-x-2">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-cyan-300">Asistente Cósmico</div>
                      </div>
                    </div>
                    <div className="flex items-center space-x-2">
