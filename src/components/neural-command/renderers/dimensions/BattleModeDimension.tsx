      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        >
          ...
          <p className="text-white/70 text-lg max-w-2xl mx-auto mt-2">
            Compite en batallas épicas de conocimiento PAES en tiempo real
          </p>
        </motion.div>

        {/* Métricas de Battle Readiness */}
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="bg-white/10 rounded-lg p-4 backdrop-blur-sm">
            <h3 className="text-red-400 font-bold text-xl mb-2">🔥 Preparación de Batalla</h3>
          ...
          <div className="bg-white/10 rounded-lg p-4 backdrop-blur-sm">
            <h3 className="text-orange-400 font-bold text-xl mb-2">🎮 Engagement</h3>
            <div className="text-3xl font-bold text-white mb-2">{gamificationEngagement}%</div>
            <div className="w-full bg-white/20 rounded-full h-2">
              <div 
                className="bg-orange-500 h-2 rounded-full transition-all duration-1000"
                style={{ width: `${gamificationEngagement}%` }}
              />
            </div>
          </div>
        </div>
        ...
        {activeBattle && (
          ...
          >
            <h3 className="text-white font-bold text-xl mb-4">🏟️ Batalla Activa</h3>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <div className="space-y-4">
            <h3 className="text-white font-bold text-xl">🎯 Crear Nueva Batalla</h3>
            <motion.div
          ...
          <div className="space-y-4">
            <h3 className="text-white font-bold text-xl">⚡ Batallas Disponibles</h3>
            <div className="space-y-2 max-h-64 overflow-y-auto">
              {availableBattles.length > 0 ? (
                ...
                <div className="bg-white/10 rounded-lg p-4 backdrop-blur-sm text-center">
                  ...
                  <div className="text-white/40 text-sm">¡Crea una nueva batalla!</div>
                </div>
              )}
            </div>
          </div>
        ...
        {userBattles.length > 0 && (
          <div>
            <h3 className="text-white font-bold text-xl mb-4">📊 Historial de Batallas</h3>
              ...
              {userBattles.slice(0, 3).map((battle) => (
                <div key={battle.id} className="bg-white/10 rounded-lg p-4 backdrop-blur-sm">
                  ...
                  <div className="text-white/70 text-sm">
                    {battle.status === 'completed' ? '✅ Completada' : '⏳ En progreso'}
                  </div>
                  {battle.winner_id && (
                    <div className="text-green-400 text-sm mt-1">
                      {battle.winner_id === battle.creator_id ? '🏆 Victoria' : '💪 Derrota'}
                    </div>
                  )}
                </div>
              ))}
