...
export const ExerciseIntersectional: React.FC<ExerciseIntersectionalProps> = ({
  ...
  const generateExercise = useCallback(async () => {
    ...
    try {
      // Simular generación de ejercicio basado en contexto interseccional
      const exercise = {
      ...
      <CardContent className="space-y-6">
        ...
        {context.financialGoals && (
          <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
            <h4 className="font-medium text-purple-900 mb-2">🎯 Recomendación Interseccional</h4>
            <p className="text-purple-800 text-sm">
              Tu rendimiento actual ({Math.round(context.crossModuleMetrics.averagePerformance)}%) 
              está alineado con tu objetivo de acceder a {context.financialGoals.targetCareer}.
              {context.crossModuleMetrics.averagePerformance < 70 && 
                " Considera practicar más para mejorar tus oportunidades."
              }
            </p>
          </div>
        )}
