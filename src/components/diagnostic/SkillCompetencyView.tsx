...
export const SkillCompetencyView = ({ skill, score }: SkillCompetencyViewProps) => {
  ...
  const getLevel = () => {
    ...
    if (scorePercent >= 20) return { label: "Básico", color: "bg-yellow-500" };
  ...
  const getRecommendation = () => {
    if (scorePercent >= 80) return "Dominas esta habilidad. Continúa practicando para mantener tu nivel.";
    if (scorePercent >= 60) return "Buen dominio. Enfócate en mejorar aspectos específicos para llegar al siguiente nivel.";
    if (scorePercent >= 40) return "Nivel adecuado. Practica regularmente para fortalecer esta habilidad.";
    if (scorePercent >= 20) return "Requiere atención. Dedica tiempo adicional a mejorar esta habilidad.";
    return "Necesita refuerzo prioritario. Comienza con ejercicios básicos y avanza gradualmente.";
  };
