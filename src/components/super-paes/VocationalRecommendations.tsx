...
export const VocationalRecommendations: React.FC<VocationalRecommendationsProps> = ({
  recomendaciones,
  onActualizar
}) => {
  const [selectedCategory, setSelectedCategory] = React.useState<'principal' | 'alternativas' | 'exploratorias'>('principal');
    ...
    <div className="space-y-8">
      ...
      >
        <div>
          ...
          <p className="text-purple-200">Carreras personalizadas según tu perfil PAES</p>
        </div>
      ...
      >
        {currentCategory.data?.map((carrera: any, index: number) => (
          <CareerCard
            key={carrera.nombre}
            carrera={carrera}
            index={index}
      ...
      <Card className="bg-gradient-to-br from-slate-800/50 to-slate-900/50 backdrop-blur-xl border-slate-600/30 hover:border-slate-500/50 transition-all duration-300">
        ...
        <CardContent>
          ...
          {expanded && (
            ...
            >
              <div>
                <div className="text-sm font-medium text-white mb-1">Descripción:</div>
              ...
              {carrera.recomendacionPersonalizada && (
                <div>
                  <div className="text-sm font-medium text-white mb-1">Recomendación Personalizada:</div>
